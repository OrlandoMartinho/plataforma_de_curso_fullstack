...
const UsersController = {
            ...
            db.query(selectQuery, [email],async (err,result)=>{
                ...
                const isPasswordValid = await bcrypt.compareSync(senha, usuario.senha);
                        ...
                        db.query(updateQuery, params, (err, result) => {
                            ...
                            return res.status(201).json({ Mensagem: "Autenticação bem-sucedida", accessToken :accessToken,id_usuario:usuario.id_usuario});                      
                        });
    ...
    editarAssinatura: async (req, res) => {
        try {
            upload(req, res, async function(err) {
                ...
                try {
                    const { accessToken, nome, numero_do_bi, localizacao, numero_de_telefone ,id_curso} = req.body;
                    const arquivo = req.file;
    
                    // Verificar se todos os campos obrigatórios estão presentes
                    if (!nome || !numero_do_bi || !numero_de_telefone || !localizacao || !accessToken||!id_curso) {
                        return res.status(400).json({ mensagem: "Campos incompletos" });
                    }
                    ...
                    if (!['.png', '.jpg', '.jpeg', '.pdf'].includes(extensao)) {
                        return res.status(400).json({ mensagem: 'Formato de arquivo inválido. Apenas arquivos PNG, JPG, JPEG, PDF são permitidos' });
                    }
    
                    const nomeArquivo = `arquivo_${id_usuario}${extensao}`;
                    const pathArquivo = `./uploads/comprovativos/${nomeArquivo}`;
    
                    fs.writeFileSync(pathArquivo, arquivo.buffer);
    
                    const comprovativo = nomeArquivo;
    
                    const updateQuery = 'UPDATE usuarios SET nome = ?, numero_do_bi = ?, localizacao = ?, numero_de_telefone = ?, assinado = ?, comprovativo = ?,data_assinatura = ?,id_curso = ? WHERE id_usuario = ?';
                    db.query(updateQuery, [nome, numero_do_bi, localizacao, numero_de_telefone, 1, comprovativo,data_assinatura,id_curso, id_usuario], (err, result) => {
                        if (err) {
                            console.log("Erro:", err);
                            return res.status(500).json({ mensagem: "Erro interno do servidor" });
                        }
                        ...
                        db.query(selectQuery, [id_usuario], async (err, result) => {
                            ...
                            // Executar a consulta SQL
                            db.query(updateTokenQuery, params, (err, result) => {
                                if (err) {
                                    console.error('Erro ao atualizar token do usuário:', err);
                                    return res.status(500).json({ mensagem: "Erro interno do servidor" });
                                }
                                ...
                                notify.addNotificacao(notificacao, 1, titulo);
                                return res.status(201).json({ mensagem: "Usuário assinado com sucesso", accessToken: accessToken });
                            });
                        });
                    });
                } catch (error) {
                    console.error('Erro ao Enviar Arquivo:', error);
                    return res.status(500).json({ mensagem: 'Erro ao enviar o arquivo: ' + error.message });
                }
            });
        } catch (error) {
    ...
    obterTodosUsuariosAssinados:async(req,res)=>{
       ...
       if(! await token.verificarTokenUsuario(accessToken)||token.usuarioId(accessToken)!=1){
        ...
        const selectQuery='SELECT * FROM usuarios where assinado = 1'
    ...
    confirmarAssinatura:async(req,res)=>{
       ...
       db.query(updateQuery,[data_assinatura,0,id_usuario],(err,result)=>{
    ...
    retornarComprovativo: async (req, res) => {
        try {
            ...
            const extensao = path.extname(nomeDoArquivo).toLowerCase();
            let formato;
            ...
            switch (extensao) {
                ...
                case '.pdf':
                    ...
                    break;
                default:
                    return res.status(400).json({ mensagem: 'Formato de arquivo inválido' });
            }
    ...
    pesquisarUsuario:async(req,res)=>{
        ...
        const selectQuery = `
            SELECT * FROM usuarios  WHERE nome LIKE ?  OR id_usuario = ? OR genero LIKE ? OR numero_de_telefone LIKE ? OR email LIKE ?`;
    
    
        db.query(selectQuery, [`%${valor}%`, `%${valor}%`, `%${valor}%`, `%${valor}%`, `%${valor}%`], async (err, result) => {
            if(err){
                console.log("Erro:"+err.message);
                return res.status(500).json({Mensagem:"Erro interno do servidor"});
            }           
            console.log(result)
            return res.status(200).json({usuarios:result[0]});
        });
    },
